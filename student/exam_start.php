 ...
 * Rules page + exam launch.
 *
 * CRITICAL SESSION RULE:
 *   session_write_close() must ONLY be called immediately before the final
 *   header("Location: exam.php") redirect — NEVER before setFlash() or
 *   verifyCsrfToken(), as those read/write $_SESSION.
 */
...
// ── Load schedule ─────────────────────────────────────────────
// Accept scheduled, active, or missed (will auto-restore if still in window)
$schedule = db()->fetchOne("
    ...
    WHERE es.id = ? AND es.student_id = ?
      AND es.status IN ('scheduled','active','missed')
      AND es.attempt_allowed = 1",
    [$scheduleId, $studentId]
);

if (!$schedule) {
    setFlash('error', 'Exam not found or you are not assigned to this exam.');
    redirect('dashboard.php');
}

// ── Time window check (PHP PKT times) ─────────────────────────
$now     = time();
$startTs = strtotime($schedule['scheduled_date'] . ' ' . $schedule['start_time']);
$endTs   = strtotime($schedule['scheduled_date'] . ' ' . $schedule['end_time']);
if (!$endTs) {
    // Fallback: calculate from duration
    $endTs = $startTs + ($schedule['duration_minutes'] * 60);
}
$windowEndTs = $endTs + 300; // +5 min grace

// If marked missed but still in window → restore
if ($schedule['status'] === 'missed' && $now >= $startTs && $now <= $windowEndTs) {
...
if ($now < $startTs) {
    ...
    setFlash('warning', "Exam has not started yet. Starts in {$h}h {$m}m (PKT: " .
        date('h:i A', $startTs) . ').');
    redirect('dashboard.php');
}
if ($now > $windowEndTs) {
...
// ── Existing attempt checks ───────────────────────────────────
$completedAttempt = db()->fetchOne(
    "SELECT id FROM exam_attempts
     WHERE schedule_id=? AND student_id=? AND status='completed'",
    [$scheduleId, $studentId]
);
if ($completedAttempt) {
...
$terminatedAttempt = db()->fetchOne(
    "SELECT id, termination_reason FROM exam_attempts
     WHERE schedule_id=? AND student_id=? AND status='terminated'",
    [$scheduleId, $studentId]
);

$inProgressAttempt = db()->fetchOne(
    "SELECT id FROM exam_attempts
     WHERE schedule_id=? AND student_id=? AND status='in_progress'",
    [$scheduleId, $studentId]
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['start_exam'])) {
    ...
    if ($inProgressAttempt) {
        // Close session lock right before navigating to exam
        session_write_close();
        header("Location: exam.php?attempt_id={$inProgressAttempt['id']}");
        exit;
    }
    ...
    $attemptId = db()->insert(
        "INSERT INTO exam_attempts
             (schedule_id, student_id, exam_id, start_time, expected_end_time, status, ip_address)
         VALUES (?,?,?,NOW(),?,?,?)",
    ...
    // Load questions into student_answers
    ...
    if (empty($questions)) {
        ...
        setFlash('error', 'This exam has no questions. Please contact your administrator.');
        redirect('dashboard.php');
    }
    ...
    foreach ($questions as $i => $q) {
        db()->execute(
            "INSERT IGNORE INTO student_answers
             ...
             VALUES (?,?,?,?,?,?)",
            [$attemptId, $studentId, $schedule['exam_db_id'],
             $q['question_id'], $q['correct_option'], $i + 1]
        );
    }

    db()->execute("UPDATE exam_schedules SET status='active' WHERE id=?", [$scheduleId]);
    logActivity('student', $studentId, 'exam_start',
        "Started exam schedule_id=$scheduleId attempt_id=$attemptId");

    // Close session lock immediately before redirecting to exam
    session_write_close();
...
<head>
    ...
    .info-strip { display:flex; flex-wrap:wrap; border-bottom:1px solid #eff0f6; }
    .info-block {
    ...
    .btn-begin:hover:not(:disabled) {
        transform:translateY(-2px); box-shadow:0 8px 24px rgba(27,94,32,.35); color:#fff;
    }
...
<div class="rules-wrap">
    ...
    <div style="text-align:center;margin-top:1.25rem;font-size:.78rem;color:#90a4ae;">
        <i class="fas fa-lock me-1"></i>All activity is monitored and recorded.
...
agreeChk.addEventListener('change', function () {
    agreeWrap.classList.toggle('checked', this.checked);
